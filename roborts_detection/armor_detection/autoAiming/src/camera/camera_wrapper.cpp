...
*/
  ...
  //  CameraGetCapability(h_camera1, &tCapability1);
    ...
    CameraSetAeState(h_camera0, false);  //设置是否自动曝光
    //CameraSetAeState(h_camera1, false);
	CameraSetExposureTime(h_camera0, 20000);
//	CameraSetExposureTime(h_camera1, 20000);

    CameraSetAnalogGain(h_camera0, 64);
  //  CameraSetAnalogGain(h_camera1, 64);
	
CameraSetLutMode(h_camera0, LUTMODE_PARAM_GEN);
//CameraSetLutMode(h_camera1, LUTMODE_PARAM_GEN);
    // 抗频闪
...
bool CameraWrapper::readRaw(cv::Mat &src0, cv::Mat &src1) {
        ...
        CameraGetImageBuffer(h_camera1, &frame_info1, &pby_buffer1, 1000) == CAMERA_STATUS_SUCCESS)
    {
        ...
        src0 = cv::cvarrToMat(iplImage0).clone();
        src1 = cv::cvarrToMat(iplImage1).clone();
